# design.md — GSM ride-booking simulation
...
2. **Xác nhận điểm đón** — xác nhận lại địa chỉ đã chọn trước khi qua bước tiếp theo
3. **Chọn loại xe** — Xe máy / Ô tô
...
4. **Xem giỏ hàng** — danh sách món đã thêm, có thể sửa/xoá
5. **Chọn ưu đãi** — danh sách ưu đãi hard-code sẵn
6. **Đặt đơn** — tóm tắt giỏ hàng + ưu đãi + nút xác nhận cuối cùng
...
---
version: alpha
name: GreenSM-design-analysis
description: An inspired interpretation of Green SM's (formerly Xanh SM) design language — an EV ride-hailing and delivery super-app brand whose web surface pairs a signature cyan (a deliberate blend of green and blue, "xanh và thông minh") against a clean white canvas, framed by a geometric display sans, accented by a single signature pill shape (radius 999px) on every interactive element, and decorated only by editorial 4:3 illustrations of riders, drivers, and city objects. Note: the cyan value below is an approximation — cross-check against Green SM's actual brand swatch/CMYK guide before shipping pixel-accurate work.
...
colors:
  ...
  canvas: "#ffffff"
  canvas-soft: "#eafafa"
  canvas-softer: "#f2fcfc"
  surface-pressed: "#cdeeee"
  link: "#0000ee"
  on-dark: "#ffffff"
  primary-dark: "#048589"
...
Green SM is an EV ride-hailing and delivery super-app — ride, food, freight, the whole urban mobility layer — and the brand's web surface signals that scale through restraint: no third colour, no accent palette, no illustration that fights the headline. The page is structurally a cyan-and-white duet, where the signature cyan `{colors.primary}` (a deliberate blend of green and blue — "xanh và thông minh") is the conversion anchor (every CTA pill, every nav login button, the footer fill) and `{colors.canvas}` white carries everything else. The only consistent decoration is a body of editorial 4:3 illustrations — riders, drivers, parking lots, cars-on-highway — that ground the marketing without leaking a second accent colour into the system.
...
**Key Characteristics:**
- A two-colour CTA hierarchy: signature cyan `{colors.primary}` pill for primary conversion targets; white `{colors.canvas}` pill (sometimes with a soft drop shadow) for secondary; subtle cyan-tinted `{colors.canvas-soft}` pill for tertiary or chip variants.
...
- A signature alternating-band rhythm: white feature card → near-black promo card (with white text and white CTA) → white feature card → cyan footer. The dark bands are NOT hero-only; they appear mid-page as promo callouts.
- A signature ride-request form card on the hero: pickup pin input + destination input + date/time chip + cyan "See prices" pill, all stacked inside a `{rounded.xl}` shadowed card.

## Colors

### Brand & Accent
- **Signature Cyan** (`{colors.primary}` — `#00B4B8`, approximate — verify against the official brand swatch): The brand's only conversion colour, and its defining mark — the deliberate blend of green (environment, clean energy) and blue (intelligence, technology) that gives Green SM its "xanh và thông minh" identity. Every primary CTA pill, the footer fill, every nav login button. The system has no secondary accent.
- **Surface Pressed** (`{colors.surface-pressed}` — `#cdeeee`): The pressed-state fill for white pills — a soft cyan-tinted grey that's used only in active / pressed states.
- **Primary Dark** (`{colors.primary-dark}` — `#048589`): A deeper cyan used on hover for the translucent white tab-toggle pill. Documented as a system colour because it appears on a recurring brand control.
...
|---|---|---|---|---|
| `{typography.display-xxl}` | 52px | 700 | 64px | Hero headline ("Go anywhere with Green SM", "Drive when you want"). |
...
### Decorative Depth
- **Ink bands as polarity-flip depth**: the brand uses near-black `{colors.ink}` mid-page bands to break the white-on-white rhythm, while the signature cyan `{colors.primary}` stays reserved for conversion (CTAs, footer) so the two dark surfaces read as distinct roles rather than competing accents. The polarity shift IS the depth cue.
...
**`button-primary`** — the canonical cyan pill, the brand's conversion target.
...
**`button-secondary`** — the white pill paired with the cyan primary.
...
**`button-large-rounded`** — the bigger cyan call-to-action used inside the ride-request flow ("Yes, help me").
- Background `{colors.primary}`, text `{colors.on-primary}`, label in `{typography.button-large}`, padding `{spacing.lg} {spacing.xl}`, shape `{rounded.xl}` 16 px (not pill — the only cyan CTA that breaks the pill rule, used in the larger form context).
...
- Background `{colors.ink}`, text `{colors.on-dark}`, padding `{spacing.2xl}`, shape `{rounded.xl}`. Used for the "Drive with Green SM" mid-page band.
...
- Background `{colors.canvas}` on light pages, switches to `{colors.ink}` on the rare dark page (e.g., a Green SM Food hero). Padding `{spacing.lg} {spacing.3xl}`.

**`nav-link`** — the link row inside `nav-bar`.
- Text `{colors.ink}`, set in `{typography.body-md-strong}` 500 weight.

**`footer`** — the signature-cyan footer band.
- Background `{colors.primary}` (the brand's conversion colour, used at full scale here), text `{colors.on-dark}`, padding `{spacing.3xl} {spacing.3xl}`. Body in `{typography.body-sm}`; column eyebrows in `{typography.body-md-strong}`.
...
**`hero-band-dark`** — the rare black hero (used on Green SM Food and driver-recruitment landing pages).
...
**`app-download-pill`** — the "Download the Green SM app" / "Download the Green SM Driver app" pill.
...
### Do
- Reserve `{colors.primary}` (signature cyan) for every primary CTA pill. One cyan pill per visible viewport is the brand's whole conversion story.
...
- Use polarity-flipped ink-black promo bands mid-page to break up white-on-white rhythm, kept visually distinct from the cyan conversion colour. The polarity shift IS the depth cue.
- Anchor every promo card with a 4:3 editorial illustration; never use generic stock imagery.

### Don't
- Don't introduce a second brand accent colour beyond the signature cyan (no orange, no separate green or blue used on its own). The blend of green and blue into one cyan IS the brand's only chromatic; splitting it back into its component colours dilutes the identity.
...
- Don't reduce the brand to its illustration system alone. The pill geometry + cyan/white duet carries the brand even without illustrations.
